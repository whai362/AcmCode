...
给出两个矩形，a1,b1,a2,b2，可以1/2切，可以2/3切，求最大相等的面积。待选1 u

细胞分裂 没次有p的概率分裂，求t秒后数目超过s的概率

给出点P和一条线段L，再给出一个点Q和一个点A，线段L绕P顺时针旋转，点A绕Q顺时针旋转，他们的角速度相等，问他们能不能碰撞。待选 u
...
给出1~n，问最大公约数为1的三元组有多少个。 待选
质数，容斥
poj 跳蚤

2 * 3 * 5 * 7 * … * P + 1 是一个素数 待选2
...
先预处理逆元1/a[i]
然后用1...n逆元的和的平方-(1/a1a1+1/a2a2+...+1/a[n]a[n])，然后再除以2，然后再乘上a1*a2*...*a[n]即可 待选3
...
给出n个数ai(0 <= n <= 1000;1 <= ai <= 1e6)，问从中选出k个使得gcd最大，输出最大的gcd值和选择的方法数。
整数因子分拆。 待选

1e3长度的大数n,m,判C(n,m)的奇偶性。
lucas定理

0 <= m <= n <= 1e18，求C(n,m) mod (2*3*5*7*11*13*17*19)
lucas定理 中国剩余定理

给出n，m，其中n表示有n个点，m表示有m对关系，对于每对关系用(x,y)来表示，求从这n个数里选出任意个数组成一个集合(集合size>=2)，求不包含任意关系对的集合有多少个。关系对的等价关系，参考bc46 c

树的字典序最小的直径 树的中心

求1~n中和m互质的数的个数，其中(1 <= n <= 1e18), (1 <= m <= 1e6)
